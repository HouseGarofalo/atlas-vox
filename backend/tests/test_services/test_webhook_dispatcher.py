...
import app.services.webhook_dispatcher as wd_module
...
@pytest.mark.asyncio
async def test_is_url_safe_public_https():
    assert await _is_url_safe("https://example.com/hook") is True


@pytest.mark.asyncio
async def test_is_url_safe_public_http():
    # Use a domain that actually resolves to a public IP
    assert await _is_url_safe("http://example.com/callback") is True


@pytest.mark.asyncio
async def test_is_url_safe_localhost_hostname():
    assert await _is_url_safe("http://localhost/hook") is False


@pytest.mark.asyncio
async def test_is_url_safe_localhost_127():
    assert await _is_url_safe("http://127.0.0.1:9000/hook") is False


@pytest.mark.asyncio
async def test_is_url_safe_private_ip_192():
    assert await _is_url_safe("http://192.168.1.100/hook") is False


@pytest.mark.asyncio
async def test_is_url_safe_private_ip_10():
    assert await _is_url_safe("http://10.0.0.1/hook") is False


@pytest.mark.asyncio
async def test_is_url_safe_private_ip_172():
    assert await _is_url_safe("http://172.16.0.1/hook") is False


@pytest.mark.asyncio
async def test_is_url_safe_internal_domain():
    assert await _is_url_safe("http://myservice.internal/hook") is False


@pytest.mark.asyncio
async def test_is_url_safe_local_domain():
    assert await _is_url_safe("http://printer.local/hook") is False


@pytest.mark.asyncio
async def test_is_url_safe_ipv6_loopback():
    assert await _is_url_safe("http://[::1]/hook") is False


@pytest.mark.asyncio
async def test_is_url_safe_link_local():
...
async def test_dispatch_with_matching_wildcard_webhook(db_session: AsyncSession):
    ...
    # Reset the singleton HTTP client so we can inject our mock
    wd_module._http_client = None

    mock_client = AsyncMock()
    mock_client.post = AsyncMock(return_value=mock_response)

    with patch.object(wd_module, "_get_client", return_value=mock_client):
